...
Primary product direction is Cloud Web: browser access, email/password Cloud session, organization tenancy.
...
`pnpm dev:cloud` provisions a disposable synthetic identity under `.tmp/workos-dev-cloud` (gitignored). Credentials stay in that ignored root. It never uses a real HUB MEDIA Cloud root or production credentials.
