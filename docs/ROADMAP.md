...
Planning Workload V1 is the living program. PLN0 corrects the canon to workload-first: `plannedEffortMinutes` plus provider queued-effort projection. It does not implement product code, API, UI, or migration. `planningWeek` and weekly `availableMinutes` are removed from V1. PLN1, PLN2, and PLN3 are not started. Scheduling is out of V1. Member DAG / composition corrections remain deferred and are not a Planning blocker.
