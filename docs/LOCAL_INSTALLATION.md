...
CLEAN_MACHINE_LIMITATIONS = NO_DISPOSABLE_WINDOWS_VM
PACKAGE_LOCAL_CLI_DIRECT_EXECUTION = BLOCKED_BY_AGENT_HOOK
SOURCE_CONTRACT = buildLocalPackage({ rebuildFrontend: true })
CANONICAL_FRONTEND_BUILD = PASS
ISOLATED_PACKAGE_E2E = PASS
NEXT_STEP = CONTROLLED_OWNER_MACHINE_LOCAL_INSTALLATION_REHEARSAL
NEXT_STEP_STARTED = NO
```

A true empty Windows VM was not used. Do not treat this as `CLEAN_MACHINE = PASS`. The next evidence tier is a controlled Owner-machine rehearsal on a new isolated Local data root. It must not use existing HUB MEDIA business data.
