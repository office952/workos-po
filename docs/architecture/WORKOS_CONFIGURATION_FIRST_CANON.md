...
```text
STATUS = CF1_AND_CF2_CF3_COMPLETE_AND_CF4_IMPLEMENTED_IN_REVIEW
...
CF4 narrow V1 is implemented in review: organization-local versions for
`LIGHTING_FRONT_LED.ledModuleQuantity`, `LIGHTING_FRONT_LED.totalLedLoadW`, and
`LIGHTING_FRONT_LED.requiredPsuCapacityW` only. Representation is a typed
structured AST. Evaluation lives only in `packages/domain`. `selectPsuUnits`
remains a code-owned engine formula. Owner acceptance, main integration, CF5,
